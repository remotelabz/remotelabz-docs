# Sandbox 

The `Sandbox` menu permits to modify the template of a device.

![Screenshot](/images/Teacher/Teacher_Sandbox.png)


## Sandbox Modification

These templates are devices used when creating a virtual laboratory.
As soon as you choose to modify an existing template, a new Sandbox lab is created. In this lab, you will only see the device on which you want to make modifications along with a DHCP Device. 

1. Start first the DHCP_Service device to start a DHCP service for your lab.
2. Start next the device you want to modify and create.

## Sandbox Export

![Screenshot](/images/Teacher/Teacher_sandbox_export.png)

To make an export, you have to :

1. Start the device. You are required to start the device at least once in order to perform the operation.
2. Do your modifications.
3. Stop the device.
4. Click on `Export` and choose a name for your new device template. Then click on export device.

 After you have clicked on the `Export Device` button, wait until the process is complete. Once done, it leaves the lab automatically and the corresponding new device template and operating system are created. 

!!! warning
    Only teachers and administrators can access this menu and modify an existing default image.

!!! info
    It's recommended to process to a manual shutdown of your device to be sure that the writing cache disk is flushed.




